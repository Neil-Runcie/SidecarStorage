...
export class DatabaseFileHandler {
    ...
    saveConfiguration() {
        ...
        let metaDataString = ObjectToText(this.configMetaData);
    ...
    saveStore(databaseName, storeName, extension, store) {
        ...
        fs.writeFileSync(storePath, ObjectToText(store), { flag: 'w' }, (err) => {
            if (err)
    ...
    loadConfiguration() {
        if (fs.existsSync(this.metaDataLocation)) {
            let loadedConfigMetaData = TextToObject(fs.readFileSync(this.metaDataLocation, "utf-8"));
        ...
        for (let file of mappedStores.values()) {
            if (file.endsWith(extension)) {
                if (fs.existsSync(databasePath + "\\" + file)) {
                    ...
                    let storeInfo = { name: file.slice(0, file.lastIndexOf(".")), fileData: TextToObject(fs.readFileSync(databasePath + "\\" + file, "utf-8")) };
